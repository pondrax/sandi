<script>
  ...
  import { user, notify } from '$lib/store';
  import { http } from '$lib/api';
	import { AirplayIcon, FeatherIcon, MoreVerticalIcon, RefreshCwIcon, Share2Icon, Trash2Icon } from 'svelte-feather-icons';
  ...
  function refreshTable(){
    // console.log(mode)
    http('get','/api/dokumen/'+ mode).then(({data})=>{
      table = {
        data :  data.data,
...
function ttd(){
  ...
  http('post','/api/dokumen/sign',data).then(response=>{
    ...
    formPassphrase.message = error.response.data.data.message +' '+error.response.data.data.error;
  })
  // upload = []
}
...
<div id="main" class="p-6">
  ...
  <div class="flex flex-wrap gap-4 justify-between mt-10 py-2">
    <div class="flex gap-2">
      ...
      <div class="w-72">
        <Input placeholder="Cari Dokumen" autocomplete="off"/>
      </div>
    </div>
    <div class="flex gap-2">
        ...
        <tbody>
          {#each table.data as d,i}            
            <tr class="hover">
              ...
              <td>
                <div class="flex gap-2">
                  <Button small primary on:click={()=>{formPreview.open=true;formPreview.prop=d}}>
                    <AirplayIcon size="20"/>
                  </Button>
                <Dropdown end>
                  <Button small>
...
<Portal>
  ...
  <div class="modal" class:modal-open={formPreview.open} on:click={()=>formPreview.open=false}>
    <div class="modal-box w-11/12 max-w-6xl" on:click|stopPropagation={()=>null}>
      ...
      <!-- svelte-ignore a11y-missing-attribute -->
      <iframe src={formPreview.preview||(formPreview.prop.file||'').replace('public/','/storage/')} class="w-full min-h-[400px]"></iframe>
      <br>
  ...
  <div class="modal" class:modal-open={formPassphrase.open} on:click={()=>formPassphrase.open=false}>
    <div class="modal-box p-6" on:click|stopPropagation={()=>null}>
      ...
      <div class="flex justify-center">
        <Button on:click={ttd}>TANDATANGAN!</Button>
      </div>
    </div>  
  </div>
